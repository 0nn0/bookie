...
const useGetBookingsQuery = (props: Props) => {
  ...
  const fetchBookings = async () => {
    let query = supabaseClient
      .from('bookings')
      .select(
        'id, start_date, end_date, status_id, status!inner(id, name), fact_table!inner(id, role_id, profile_id, property_id, profiles(id, first_name, last_name, avatar_url, updated_at), properties(id, name))'
      );
